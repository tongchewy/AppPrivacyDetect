...
class SimulateClick:
    ...
    def screencap(self):
        ...
        device_id_cmd = "-s {}".format(
            self.device_id) if self.device_id is not None else ""
        shells = [
            'adb {} shell rm {}/screen_tmp.png'.format(device_id_cmd,
                                                       self.screen_save_path),
            'adb {} shell screencap -p {}/screen_tmp.png'.format(
                device_id_cmd, self.screen_save_path),
            'adb {} pull {}/screen_tmp.png {}'.format(device_id_cmd,
                                                      self.screen_save_path,
                                                      self.path),
            'adb {} shell rm {}/screen_tmp.png'.format(device_id_cmd,
                                                       self.screen_save_path)
        ...
        try:
            ...
            if result:
                print_msg(
                    "获取当前屏幕成功，同意隐私协议请鼠标左键点击对应同意位置。如无隐私协议，继续获取屏幕请按 n, 退出请按 q ")
            else:
    ...
    def touchscreen(self, x, y):
        ...
        device_id_cmd = "-s {}".format(
            self.device_id) if self.device_id is not None else ""
        shell = 'adb {} shell input tap {x} {y}'.format(device_id_cmd,
                                                        x=x,
                                                        y=y)
        try:
...
if __name__ == '__main__':
    sc = SimulateClick(get_frida_device()["device"].id, '/data/local/tmp',
                       'screen_shot.png')
    sc.run()
