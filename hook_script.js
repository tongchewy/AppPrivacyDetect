...
function getFileMessage() {
    ...
    // ]);
    hook('java.io.File', [
        { 'methodName': 'mkdirs', 'action': action, 'messages': '尝试写入sdcard创建' },
        { 'methodName': 'mkdir', 'action': action, 'messages': '尝试写入sdcard创建' }
    ]);
}
...
function getSensor() {
    ...
    hook('android.hardware.SensorManager', [
        ...
        { 'methodName': 'registerListener', 'action': action, 'messages': '获取传感器信息' },
        { 'methodName': 'getDefaultSensor', 'action': action, 'messages': '获取传感器信息' },
    ]);
