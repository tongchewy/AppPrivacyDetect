        ...
        if message['type'] == 'send':
            ...
            if data["type"] == "notice":
                ...
                if is_show:
                    ...
                    print(
                        "[*] {0}，APP行为：{1}、行为主体：{2}、行为描述：{3}、传入参数：{4}".format(
                            alert_time, action, subject_type, messages,
                            arg.replace('\r\n', '，')))
                    print("[*] 调用堆栈：")
                    print(stacks)
                    print(
                ...
                if execl_file:
                    ...
                    execl_data.append({
                        ...
                        "同意隐私政策" + privacy_policy_status.value,
                    })
    ...
    try:
        ...
        if isHook:
            ...
            def stop(signum, frame):
                ...
                if execl_file:
                    global execl_data
                    write_xlsx(execl_data, f"./excels/{execl_file}")
                exit()
        ...
        print_msg("hook_script.js脚本错误，请排查")
        print_msg(e)
...
def agree_privacy(privacy_policy_status, device_id):
    try:
        ...
        sc = SimulateClick(device_id, screen_save_path, 'screen_shot.png')
        screencap_result = sc.run()
        if screencap_result:
            result = sc.get_result()
            while result == 1:
                sc = SimulateClick(device_id, screen_save_path,
                                   'screen_shot.png')
                sc.run()
                result = sc.get_result()
            if result == 2:
...
if __name__ == '__main__':
    ...
    last_screen_shot = os.path.join(os.getcwd(), "screen_shot.png")
    if not os.path.isfile(last_screen_shot):
        last_screen_shot = resource_path("screen_shot.png")
    ...
    parser.add_argument(
        "package", help="APP_NAME or process ID ex: com.test.demo01 、12345")
    ...
    parser.add_argument("--file",
                        ...
                        metavar="<path>",
                        required=False,
                        help="Name of Excel file to write")
    parser.add_argument("--isattach",
                        "-ia",
                        required=False,
                        action="store_const",
                        default=False,
    ...
    parser.add_argument(
        ...
        const=True,
        help=
        "close the privacy policy. after closing, default status is agree privacy policy"
    )
    ...
    module_group.add_argument(
        ...
        required=False,
        help=
        "Detect the specified module,Multiple modules are separated by ',' ex:phone,permission"
    )
    module_group.add_argument(
        "--nouse",
        "-nu",
    ...
    parser.add_argument(
        ...
        required=False,
        help=
        "use device with given serial(device id), you can get it by exec 'adb devices'"
    )
    parser.add_argument(
        "--host",
        "-H",
    ...
    parser.add_argument(
        ...
        "-es",
        required=False,
        help="load external frida script js, default: ./hook_script.js")
    ...
    if args.noprivacypolicy or args.isattach:
        ...
        did = frida_device['did'] if frida_device['did'] else frida_device[
            "device"].id
        agree_privacy_process = Process(target=agree_privacy,
                                        args=(privacy_policy_status, did))
    ...
    frida_hook(frida_device, process, use_module, args.time, args.noshow,
               args.file, args.isattach, args.external_script)
